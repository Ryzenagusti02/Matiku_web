...
const Pengaturan: React.FC<PengaturanProps> = ({ user, onProfileUpdate }) => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (profileImage) {
                ...
                avatar_url = `${urlData.publicUrl}?t=${new Date().getTime()}`; // Add timestamp to break cache
            }
            ...
            // Update username in profiles table too for consistency
            const { error: profileUpdateError } = await supabase.from('profiles').update({ username: username.toLowerCase() }).eq('id', user.id);
