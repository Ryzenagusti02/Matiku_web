...
const Analitik: React.FC<AnalitikProps> = ({ students }) => {
    ...
    const getAverageScore = (scores: Student['scores']) => {
        ...
        const allScores = Object.values(scores).map((s: Score) => s.score);
    ...
    const materiAverage = useMemo(() => {
        ...
        filteredStudents.forEach(student => {
            if (student.scores) {
                Object.values(student.scores).forEach((score: Score) => {
                    if (!materiPerformance[score.materi]) {
                        materiPerformance[score.materi] = { scores: [], count: 0 };
                    }
                    materiPerformance[score.materi].scores.push(score.score);
                    materiPerformance[score.materi].count++;
                });
            }
        });
