...
import { Student, Module, Score, Notification } from '../types';
    ...
    const renderPage = () => {
        switch (currentPage) {
            case 'dashboard':
                ...
                const allScores = students.flatMap(s => (s.scores ? Object.values(s.scores).map((score: Score) => score.score) : []));
            ...
            case 'pengaturan':
                return <Pengaturan user={user} onProfileUpdate={onProfileUpdate} />;
            default:
                return <Dashboard 
                    totalSiswa={students.length} 
                    totalModul={modules.length}
